...
# Charger les données
file_path = r'Base de données.csv'
...
def rechercher():
    ...
    critere = st.sidebar.selectbox(
        "Choisissez un critère de recherche",
        ["Nom du film", "Nom de l'acteur", "Année", "Genre"]
    )
    
    # Champ de saisie pour les genres avec suggestions
    if critere == 'Genre':
        valeur = st.sidebar.selectbox(
            "Choisissez un genre",
            ["Action", "Comedy", "Drama", "Adventure", "Horror", "Science Fiction", "Romance", "Thriller", "History", " Documentary", "War"]
        )
    else:
        valeur = st.sidebar.text_input(f"Entrez la valeur pour {critere.lower()}")
...
page = st.sidebar.selectbox("Menu Cinéma 🎬", ["Vidéo de Présentation", "Qui sommes-nous", "Votre cinéma en un clic", "Recherche Avancée"])
if page == "Vidéo de Présentation":
    page_ephemere()
elif page == "Qui sommes-nous":
    page_accueil()
elif page == "Votre cinéma en un clic":
    page_films()
